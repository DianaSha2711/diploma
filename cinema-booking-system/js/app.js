        ...
        for (const seance of seances) {
            ...
            if (!uniqSeance) {
                aUniqSeances.push({
                    ...
                    aTimes_halls: [{ time: seance.seance_time, hall: seance.seance_hallid }]
                })
            } else {
                uniqSeance.aTimes_halls.push({ time: seance.seance_time, hall: seance.seance_hallid })
            }
        }
...
function createSeanceCard(seance) {
    ...
    seance.aTimes_halls.forEach(time_hall => {
        ...
        if (hall) {
            hall.aTimes.push(time_hall.time);
        } else {
            aHalls.push({id:time_hall.hall, aTimes:[time_hall.time]});
        }
    });
    ...
    aHalls.forEach(hall => {
        ...
        hall.aTimes.forEach(time => {
            sTimes += `<button class="seance-time">${time}</button>`
        })
        sTimes += `</div>`
    });
...
async function openBookingModal(seanceId) {
    try {
        ...
        currentSeance = await cinemaAPI.getSeance(seanceId);
        if (!currentSeance) throw new Error('Сеанс не найден');


        currentHall = await cinemaAPI.getHall(currentSeance.hall_id);
...
function updateBookingModalInfo() {
    if (!currentSeance || !currentHall) return;
    ...
    if (title) {
        title.textContent = `${currentSeance.film?.title || 'Фильм'} — ${currentSeance.time}`;
    }

    const hallInfo = modal.querySelector('.hall-info');
    if (hallInfo) {
        hallInfo.textContent = `${currentHall.name} | ${currentHall.rows} рядов × ${currentHall.seatsPerRow} мест`;
    }
}

function generateHallLayout() {
    const container = document.getElementById('hallLayout');
    if (!container || !currentHall) return;
    ...
    for (let row = 1; row <= currentHall.rows; row++) {
        ...
        for (let seat = 1; seat <= currentHall.seatsPerRow; seat++) {
            const seatElement = createSeatElement(row, seat);
...
function createSeatElement(row, seat) {
    ...
    const isVIP = currentHall.vipRows?.includes(row) || false;
    if (isVIP) {
        seatElement.classList.add('vip');
        seatElement.dataset.price = currentSeance?.vip_price || currentSeance?.price || 0;
    } else {
        seatElement.classList.add('available');
        seatElement.dataset.price = currentSeance?.price || 0;
    }
    ...
    selectedSeats.forEach(seat => {
        seatsHtml += `
            <div class="selected-seat-item">
                ...
                ${seat.isVIP ? '<span class="badge bg-warning text-dark">VIP</span>' : ''}
                - ${seat.price} ₽
            </div>
        `;
        total += seat.price;
